...
export function SocialLoginButton({ provider, onClick, disabled = false }: SocialLoginButtonProps) {
  ...
  const getButtonStyle = () => {
    switch (provider) {
      case 'google':
        return 'bg-white text-slate-800 hover:bg-slate-100 border border-slate-300 shadow-sm hover:shadow transition-all';
      case 'linkedin':
        return 'bg-[#0077B5] text-white hover:bg-[#006699] shadow-sm hover:shadow transition-all';
      default:
        return '';
    }
  };
  ...
  const getIcon = () => {
    switch (provider) {
      case 'google':
        return (
          <div className="flex items-center justify-center bg-white rounded-full w-5 h-5 mr-3">
  ...
  return (
    <Button
      ...
      variant="outline"
      className={cn(
        'w-full flex items-center justify-center py-2.5 font-medium rounded-lg',
        getButtonStyle()
      )}
      onClick={handleClick}
